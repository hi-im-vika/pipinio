/**
 * CPiPinIO.cpp - GPIO control on the Raspberry Pi
 * 2025-04-02
 * vika <https://github.com/hi-im-vika>
 */

#include "../include/CPiPinIO.hpp"
...
bool CPiPinIO::init_gpio(const std::vector<int> &input_pins, std::vector<int> &output_pins) {
    if (gpioInitialise() < 0) {
...
bool CPiPinIO::init_i2c(i2c_ch ch, uint address) {
    int *temp_handle;
    bool *temp_status;
    switch (ch) {
        case i2c_ch::CH0:
...
bool CPiPinIO::init_pca9685(i2c_ch ch, uint address) {
...
bool CPiPinIO::init_hmc5883l(i2c_ch ch, uint address) {
    if (!init_i2c(ch, address)) {
        return false;
    }
...
bool CPiPinIO::init_mpu6050(i2c_ch ch, uint address) {
    if (!init_i2c(ch, address)) {
        return false;
    }
...
void CPiPinIO::queue_new_gc_data(std::string &data) {
    _gc_queue.emplace(data);
}

std::vector<int> CPiPinIO::get_gc_values() {
    return _gc_values;
}

void CPiPinIO::process_gc(std::string &gc) {
    std::stringstream ss;
    std::string temp;
    ss.str(gc);
    int idx = 0;
...
void CPiPinIO::thread_process_gc(CPiPinIO *who_called) {
...
bool CPiPinIO::get_i2c_status(i2c_ch ch) {
    switch (ch) {
        case i2c_ch::CH0:
...
int *CPiPinIO::get_i2c_handle(i2c_ch ch) {
    switch (ch) {
        case i2c_ch::CH0:
...
bool CPiPinIO::i2c_write_byte(i2c_ch ch, uint reg, uint val) {
...
bool CPiPinIO::i2c_read_byte(i2c_ch ch, uint reg, uint8_t &data) {
    if (!get_i2c_status(ch)) return false;
...
bool CPiPinIO::i2c_read_block(i2c_ch ch, uint reg, std::vector<char> &buf, int bytes) {
    if (!get_i2c_status(ch)) return false;
...
bool CPiPinIO::i2c_write_block(i2c_ch ch, uint reg, std::vector<char> &buf) {
    if (!get_i2c_status(ch)) return false;
...
bool CPiPinIO::i2c_write_word(i2c_ch ch, uint reg, uint word) {
    if (!get_i2c_status(ch)) return false;
